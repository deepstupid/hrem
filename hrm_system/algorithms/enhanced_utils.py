...
def create_dataloader(training_config: TrainingConfig, data_path: str, split: str, rank: int, world_size: int, **kwargs):
    ...
    dataloader = DataLoader(
        dataset,
        batch_size=None,
        num_workers=2,  # Increased from 1 for better data loading
        prefetch_factor=16,  # Increased from 8 for better prefetching
        pin_memory=True,
...
def compute_lr(base_lr: float, training_config: TrainingConfig, train_state: TrainState, schedule_type: str = "cosine"):
    """Enhanced learning rate computation with multiple schedule options."""
...
def train_batch(training_config: TrainingConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    ...
    use_amp = getattr(training_config, 'use_amp', True) and device.type == "cuda"
    scaler = getattr(train_state, 'scaler', None)
    ...
    for optim, base_lr in zip(train_state.optimizers, train_state.optimizer_lrs):
        # Use linear schedule for more stable training
        lr_this_step = compute_lr(base_lr, training_config, train_state, schedule_type="linear")
