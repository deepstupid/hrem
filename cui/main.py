...
def _run_evaluation(ctx, challenge_id, models, patience, smoke_test=False):
    ...
    available_challenges = {c.id: c for c in get_available_challenges(challenge_registry)}
    ...
    available_models = get_available_models(config_manager).keys()
...
def run_interactive(ctx):
    ...
    selected_challenge = _select_challenge(ctx)
    selected_models = _select_models(ctx)
    patience = _get_patience_level(ctx)

    _run_evaluation(ctx, selected_challenge.id, selected_models, patience)
...
def run_once(ctx, challenge_id, models, patience, smoke_test):
    ...
    _run_evaluation(ctx, challenge_id, list(models), patience, smoke_test)
