...
class CUIProgressHandler(ProgressHandler):
    ...
    def on_progress(self, event_type: str, data: Dict[str, Any]):
        ...
        if event_type == 'start_phase':
            self.current_phase = data.get('phase', 'Unknown Phase')
            self.console.rule(f"[bold cyan]Phase: {self.current_phase.replace('_', ' ').title()}[/bold cyan]")

        elif event_type == 'start_algorithm':
            self.current_algorithm = data.get('algorithm', 'Unknown Algorithm')
            self.console.print(f"▶️  Running algorithm: [bold green]{self.current_algorithm}[/bold green]")
            ...
            if metrics:
                table = Table(title=f"{alg} Final Metrics")
            ...
            self.console.print(f"  Trial finished. Loss: {loss:.4f}")

        elif event_type == 'insights_generated':
            self.console.rule("[bold green]🔬 Scientific Insights Generated[/bold green]")
            insights = data.get('insights', [])
            report_path = data.get('report_path')

            for insight in insights:
                self.console.print(Panel(insight, title="Insight", border_style="green"))
